...
void YoshiValley::BeginPlay() {
    ...
    struct ActorSpawnData* treeSpawns =
        (struct ActorSpawnData*) LOAD_ASSET_RAW(d_course_yoshi_valley_tree_spawn);
    for (struct ActorSpawnData* entry = treeSpawns; entry->pos[0] != END_OF_SPAWN_DATA; entry++) {
        SpawnActor<ATree>(FVector(entry->pos[0], entry->pos[1], entry->pos[2]), TreeKind::YoshiValley);
    }
